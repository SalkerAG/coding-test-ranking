...
public class PictureScoreRule implements ScoreRule {
    ...
    private static final Integer PICTURE_DIFFERENT_EXT_SCORE = 0;
            ...
            ad.getPictures()
                    .forEach(
                            pic -> {
                                ...
                                if (picture.getQuality().equals("SD"))
                                    ad.setScore(PICTURE_SD_SCORE + score);
                                else if (picture.getQuality().equals("HD"))
                                    ad.setScore(PICTURE_HD_SCORE + score);
                                else ad.setScore(PICTURE_DIFFERENT_EXT_SCORE + score);
                            });
        } else {
